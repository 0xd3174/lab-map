<script lang="ts">
  ...
  import { ROOMS, GAP_SIZE, LAB_WIDTH, LAB_HEIGHT, DOORS, CELL_SIZE } from "./../data/data";
  ...
  onMount(() => {
    ...
    panzoomInstance.on("zoom", () => (currentZoom = panzoomInstance.getTransform().scale));
...
<div>
  ...
  >
    <!-- Dot pattern -->
    <pattern id="grid" width={CELL_SIZE / 2} height={CELL_SIZE / 2} patternUnits="userSpaceOnUse">
    ...
    />
    <rect x={GAP_SIZE} y={GAP_SIZE} width={LAB_WIDTH} height={LAB_HEIGHT} fill="url(#grid)" />

    <svg x={GAP_SIZE} y={GAP_SIZE} width={LAB_WIDTH} height={LAB_HEIGHT}>
      {#each ROOMS as room}
        <svg x={room.x} y={room.y} width={room.w} height={room.h} viewBox="0 0 {room.w} {room.h}">
          <!-- Walls -->
          <rect width="100%" height="100%" fill="none" stroke="#94a3b8" stroke-width="4" />
